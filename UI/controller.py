...
class Controller:
    ...
    def handleCreaGrafo(self, e):
        ...
        self._view._txt_result.controls.append(
            ft.Text(f"Grafo creato correttamente"
                    f"Il grafo costituito ha {n} nodi e {m} archi"))
    ...
    def handleDettagli(self, e):
        ...
        self._view._txt_result.controls.clear()
        self._view._txt_result.controls.append(
            ft.Text(f"Il nodo {self._choiceTeam} ha {len(viciniTuple)}", color="green"))
        self._view._txt_result.controls.append(
            ft.Text("Di seguito una lista ordinata dei vicini:")
        )
        for t in viciniTuple:
            self._view._txt_result.controls.append(
                ft.Text(f"{t[0]} - peso {t[1]}")
            )
        self._view.update_page()
    ...
    def handleYearSelection(self,e):
        ...
        #tutti i team che hanno giocato quell'anno, e stamparli nel textfield e anche riempire il dropdown
        ...
        self._view._txtOutSquadre.controls.clear()
        self._view._txtOutSquadre.controls.append(
            ft.Text(f"Per il {self._view._ddAnno.value} anno sono iscritte al campionato {len(teams)} squadre")
        )
        for t in teams:
            self._view._txtOutSquadre.controls.append(
                ft.Text(t))
